...
struct BaseBezierCrv : public Element<Derived> {
    // Types
    ...
    explicit BaseBezierCrv(Eigen::EigenBase<EigenType> & nodes) :p_nodes(nodes.derived().template cast<typename Base::Scalar>()) {}

    /** Constructor from an Eigen matrix containing the positions of the segment's nodes */
    template<typename EigenType, REQUIRES(EigenType::RowsAtCompileTime == NumberOfNodesAtCompileTime)>
    explicit BaseBezierCrv(const Eigen::EigenBase<EigenType> & nodes) :p_nodes(nodes.derived().template cast<typename Base::Scalar>()) {}
    ...
    explicit BaseBezierCrv(const Eigen::Ref<EigenType, Options, StrideType> & nodes) : p_nodes(nodes.derived().template cast<typename Base::Scalar>()) {}
    ...
    >
    explicit BaseBezierCrv(const WorldCoordinates & first_node, Nodes&&...remaining_nodes)
    {
