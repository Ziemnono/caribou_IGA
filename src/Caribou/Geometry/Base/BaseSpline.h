...
struct BaseSpline : public Element<Derived> {
    // Types
    ...
    explicit BaseSpline(Eigen::EigenBase<EigenType> & nodes) :p_nodes(nodes.derived().template cast<typename Base::Scalar>()) {}

    /** Constructor from an Eigen matrix containing the positions of the segment's nodes */
    template<typename EigenType, REQUIRES(EigenType::RowsAtCompileTime == NumberOfNodesAtCompileTime)>
    explicit BaseSpline(const Eigen::EigenBase<EigenType> & nodes) :p_nodes(nodes.derived().template cast<typename Base::Scalar>()) {}
    ...
    explicit BaseSpline(const Eigen::Ref<EigenType, Options, StrideType> & nodes) : p_nodes(nodes.derived().template cast<typename Base::Scalar>()) {}
    ...
    >
    explicit BaseSpline(const WorldCoordinates & first_node, Nodes&&...remaining_nodes)
    {
