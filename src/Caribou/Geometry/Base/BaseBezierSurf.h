...
struct BaseBezierSurf : public Element<Derived> {
    // Types
    ...
    explicit BaseBezierSurf(Eigen::EigenBase<EigenType> & nodes) :p_nodes(nodes.derived().template cast<typename Base::Scalar>()) {}

    /** Constructor from an Eigen matrix containing the positions of the segment's nodes */
    template<typename EigenType, REQUIRES(EigenType::RowsAtCompileTime == NumberOfNodesAtCompileTime)>
    explicit BaseBezierSurf(const Eigen::EigenBase<EigenType> & nodes) :p_nodes(nodes.derived().template cast<typename Base::Scalar>()) {}
    ...
    explicit BaseBezierSurf(const Eigen::Ref<EigenType, Options, StrideType> & nodes) : p_nodes(nodes.derived().template cast<typename Base::Scalar>()) {}
    ...
    >
    explicit BaseBezierSurf(const WorldCoordinates & first_node, Nodes&&...remaining_nodes)
    {
