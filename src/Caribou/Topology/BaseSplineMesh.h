...
namespace caribou::topology {
    ...
    struct BaseSplineDomain;

    /**
     * A Splinemesh is a discrete view of a space by the mean of nodes. It can contain one or many domains, which are
     * responsible for maintaining the topological relation between the nodes of a subspace of the mesh.
     */
    struct BaseSplineMesh {
        // Stores the weights of every node
//        using WeightsContainer = Eigen::Matrix<FLOATING_POINT_TYPE, 1, Eigen::Dynamic, Eigen::RowMajor>;

        /** Destructor */
        virtual ~BaseSplineMesh() = default;
...
//        [[nodiscard]] virtual auto weight(const UNSIGNED_INTEGER_TYPE & node_id) const -> WeightsContainer = 0;
         ...
         */
        virtual auto add_domain(BaseSplineDomain * domain) -> BaseSplineDomain * = 0;
         ...
         * @return The domain that is now linked to this mesh.
         */
        virtual auto add_domain(BaseSplineDomain * domain, const std::string & name) -> BaseSplineDomain * = 0;
    };
}
