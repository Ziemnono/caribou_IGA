...
#include <Caribou/Topology/BaseSplineMesh.h>
...
//                               references to external buffers.
    ...
    public:
        ...
        /**
         * Eigen vector to store the node weights of the Splinemesh
         */
        ...
        using Self = SplineMesh<WorldDimension, NodeContainer_t>;
        ...
        using SplineDomain = SplineDomain<Element, NodeIndex>;
         ...
         */
        explicit SplineMesh(const std::vector<WorldCoordinates> & positions, const std::vector<FLOATING_POINT_TYPE> & weights)
        {
...
//        }
        ...
        /*! Copy constructor */
        SplineMesh(const SplineMesh & other)
        : p_nodes (other.p_nodes), p_weights (other.p_weights), p_domains {}
        {
            // Copy domains
            p_domains.reserve(other.p_domains.size());
            for (const auto & p : other.p_domains) {
                const std::string & domain_name = p.first;
                BaseSplineDomain * cloned_domain_ptr  = p.second->clone();
                p_domains.emplace_back(domain_name, cloned_domain_ptr);
            }
        }

        /*! Move constructor */
        SplineMesh(SplineMesh && other) noexcept
        : SplineMesh() {
            swap(*this, other);
        }

        /*! copy-and-swap assigment (valid for both copy and move assigment) */
        auto operator=(SplineMesh other) noexcept -> SplineMesh &
        {
            swap(*this, other);
            return *this;
        }
         ...
         */
        [[nodiscard]]
        inline auto domains() const -> std::vector<std::pair<std::string, const BaseSplineDomain *>> {
            std::vector<std::pair<std::string, const BaseSplineDomain *>> domains;
         ...
         * */
        [[nodiscard]]
        inline auto domain(const UNSIGNED_INTEGER_TYPE & i) const -> const BaseSplineDomain * {
            caribou_assert(
                (i < p_domains.size()) &&
        ...
        [[nodiscard]]
        inline auto domain(const std::string & name) const -> const BaseSplineDomain * {
            for (const auto & d : p_domains) {
                if (d.first == name) {
                    return d.second.get();
                }
...
// ========================== IGA Specialization ==============================
        ...
        add_domain(const std::string & name, Args ...args) {
            ...
            auto domain_ptr = new SplineDomain<Element, NodeIndex>(this, std::forward<Args>(args)...);
            add_domain(domain_ptr, name);
        ...
        inline auto add_domain(const std::string & name, Args ...args) -> SplineDomain<Element> * {
            static_assert(geometry::traits<Element>::Dimension == Dimension, "The dimension of the mesh doesn't match the dimension of the element type.");
         ...
         */
        template<typename Element, typename... Args>
        inline auto add_domain(const char * name, Args ...args) -> SplineDomain<Element> * {
         ...
         */
        template<typename Element, typename... Args>
        inline auto add_domain(Args ...args) -> SplineDomain<Element> * {
            static_assert(geometry::traits<Element>::Dimension == Dimension, "The dimension of the mesh doesn't match the dimension of the element type.");
            auto domain_ptr = new SplineDomain<Element>(this, std::forward<Args>(args)...);
         ...
         */
        inline auto add_domain(BaseSplineDomain * domain, const std::string & name) -> BaseSplineDomain * override {
            if (domain->mesh() == this) {
         ...
         * \copydoc caribou::topology::BaseMesh::add_domain
         */
        inline auto add_domain(BaseSplineDomain * domain) -> BaseSplineDomain * override {
         ...
         * @return bool True on success, false otherwise
         */
        inline auto remove(const BaseSplineDomain * domain) -> bool {
            auto it = p_domains.begin();
        ...
        */
        ...
        friend void swap(SplineMesh & first, SplineMesh& second) noexcept
        {
            // enable ADL
            using std::swap;
    ...
    private:
         ...
         */
        std::vector<std::pair<std::string, std::unique_ptr<BaseSplineDomain>>> p_domains;
    };
} // namespace caribou::topology
...
namespace caribou::topology {
    ...
    SplineMesh(const std::vector<Eigen::Matrix<Real, 1, WorldDimension>> &) ->
    SplineMesh <
        WorldDimension,
    ...
    template <typename Derived>
    SplineMesh(const Eigen::MatrixBase<Derived> &) ->
    SplineMesh <
