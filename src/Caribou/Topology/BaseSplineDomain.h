...
namespace caribou::topology {
     ...
     *
     * In a domain, all the elements are of the same type.
     */
    struct BaseSplineDomain {

        /** Destructor */
        virtual ~BaseSplineDomain() = default;
         ...
         */
        [[nodiscard]] virtual auto mesh() const -> const BaseSplineMesh * = 0;
         ...
         */
        [[nodiscard]] virtual BaseSplineDomain * clone() const = 0;

        /*!
         * Attach this Domain to the given Mesh.
         */
        virtual void attach_to(const BaseSplineMesh * mesh) = 0;
    };
}
