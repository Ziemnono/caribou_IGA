...
CaribouSplineMass<Element>::CaribouSplineMass()
...
template<typename Element>
void CaribouSplineMass<Element>::init() {
    ...
    if (not d_topology_container.get()) {
        ...
        auto caribou_containers = context->template getObjects<CaribouSplineTopology>(BaseContext::Local);
...
//            p_topology->init();
            ...
            std::cout << "caribou_topology found\n";
...
auto CaribouSplineMass<Element>::canCreate(Derived *o, sofa::core::objectmodel::BaseContext *context,
...
template<typename Element>
void CaribouSplineMass<Element>::assemble_mass_matrix() {
    assemble_mass_matrix(*this->mstate->read (sofa::core::ConstVecCoordId::restPosition()));
}

template<typename Element>
void CaribouSplineMass<Element>::assemble_mass_matrix(const sofa::Data<VecCoord> & x0) {
    using namespace sofa::core::objectmodel;
...
template<typename Element>
template<typename Derived>
void CaribouSplineMass<Element>::assemble_mass_matrix(const Eigen::MatrixBase<Derived> & x0) {
    ...
    sofa::helper::AdvancedTimer::stepBegin("CaribouSplineMass::update_mass_matrix");
    for (int element_id = 0; element_id < static_cast<int>(nb_elements); ++element_id) {
        ...
        for (const auto & gauss_node : gauss_nodes_of(element_id)) {
            ...
            element_mass += density*w*detJ*J2;
            ...
            for (std::size_t i = 0; i < NumberOfNodesPerElement; ++i) {
                ...
                for (std::size_t j = i; j < NumberOfNodesPerElement; ++j) {
                    ...
                    Me.template block<2, 2>(i*Dimension, j*Dimension)
                      .template diagonal<0>()
                      += Vector<2>::Constant(density*Ni*Nj*w*detJ*J2);
                }
            }
        }
    ...
    sofa::helper::AdvancedTimer::stepEnd("CaribouSplineMass::update_mass_matrix");
}

template <typename Element>
auto CaribouSplineMass<Element>::get_gauss_nodes(const std::size_t & /*element_id*/, const Element & element) const -> GaussContainer {
...
template <typename Element>
void CaribouSplineMass<Element>::initialize_elements()
{
    using namespace sofa::core::objectmodel;

    sofa::helper::AdvancedTimer::stepBegin("CaribouSplineMass::initialize_elements");
    ...
    sofa::helper::AdvancedTimer::stepEnd("CaribouSplineMass::initialize_elements");
}

template<typename Element>
void CaribouSplineMass<Element>::addForce(const sofa::core::MechanicalParams * /*mparams*/, CaribouSplineMass::DataVecDeriv & d_f,
                                    const CaribouSplineMass::DataVecCoord & d_x, const CaribouSplineMass::DataVecDeriv & /*d_v*/) {
    const auto density = d_density.getValue();
    if (density < std::numeric_limits<Real>::epsilon()) {
        return;
    }
    ...
    // Start the timer
    sofa::helper::ScopedAdvancedTimer _t_ ("CaribouSplineMass::addForce");
    ...
    for (int element_id = 0; element_id < static_cast<int>(nb_elements); ++element_id) {
        ...
        for (const auto & gauss_node : gauss_nodes_of(element_id)) {
            ...
            Eigen::Matrix<Real, 2, 1> force;
            force[0] = g[0]*density*w*detJ * J2;
            force[1] = g[1]*density*w*detJ * J2;
...
template<typename Element>
void CaribouSplineMass<Element>::addMToMatrix(sofa::defaulttype::BaseMatrix * matrix, SReal mFact, unsigned int & offset) {
    // Start the timer
    sofa::helper::ScopedAdvancedTimer _t_ ("CaribouSplineMass::addMToMatrix");
...
template<typename Element>
void CaribouSplineMass<Element>::addGravityToV(const sofa::core::MechanicalParams * mparams, CaribouSplineMass::DataVecDeriv & d_v) {
    ...
    const auto g_sofa = this->getContext()->getGravity();
    Eigen::Map<const Eigen::Matrix<Real, 3, 1>> gg (g_sofa.data());

    Eigen::Matrix<Real, 2, 1> g;
    g[0] = gg[0];
    g[1] = gg[1];
    // Get dt
    ...
    // dt*g
    const Eigen::Matrix<Real, 1, 2> hg = (dt*g).transpose();
    sofa::helper::ScopedAdvancedTimer _t_ ("CaribouSplineMass::addGravityToV");
    v.rowwise() += hg;
}

template<typename Element>
void CaribouSplineMass<Element>::accFromF(const sofa::core::MechanicalParams * /*mparams*/, CaribouSplineMass::DataVecDeriv & d_a,
                                    const CaribouSplineMass::DataVecDeriv & d_f) {
    // Map SOFA vectors to Eigen matrices
...
template<typename Element>
void CaribouSplineMass<Element>::addMDx(const sofa::core::MechanicalParams * /*mparams*/, CaribouSplineMass::DataVecDeriv &d_f,
                                  const CaribouSplineMass::DataVecDeriv &d_dx, SReal factor) {
