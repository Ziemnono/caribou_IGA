...
class CaribouSplineMass : public sofa::core::behavior::Mass<typename SofaVecType<caribou::geometry::traits<Element>::Dimension>::Type> {
public:
    SOFA_CLASS(SOFA_TEMPLATE(CaribouSplineMass, Element), SOFA_TEMPLATE(sofa::core::behavior::Mass, typename SofaVecType<caribou::geometry::traits<Element>::Dimension>::Type));
    ...
    using Link = sofa::core::objectmodel::SingleLink<CaribouSplineMass<Element>, ObjectType, sofa::core::objectmodel::BaseLink::FLAG_STRONGLINK>;
    ...
    static auto templateName(const CaribouSplineMass<DerivedElement>* = nullptr) -> std::string {
        return SofaCaribou::topology::CaribouSplineTopology<DerivedElement>::templateName(nullptr);
    }
    ...
    [[nodiscard]] inline
    auto topology() const noexcept -> typename SofaCaribou::topology::CaribouSplineTopology<Element>::SPtr {
        return p_topology;
    }
...
private:
    ...
    /// BaseMeshTopology (SOFA topology container), or a CaribouSplineTopology.
    ...
    // Private variables
    /// Pointer to a CaribouSplineTopology. This pointer will be null if a CaribouSplineTopology
    /// is found within the scene graph and linked using the d_topology_container data
    /// parameter. Otherwise, if a compatible SOFA's topology (see SofaCaribou::topology::CaribouSplineTopology::mesh_is_compatible())
    /// is found and linked, an internal CaribouSplineTopology component will be created
    /// and its pointer will be stored here.
    typename SofaCaribou::topology::CaribouSplineTopology<Element>::SPtr p_topology;
